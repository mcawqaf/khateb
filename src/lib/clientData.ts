...
export async function submitRegistration(payload: Record<string, unknown>): Promise<Registration> {
  ...
  try {
    ...
    // API endpoint unavailable, fall back to direct Supabase client
  }

  // 2. Direct Supabase Fallback (Serverless / GitHub Pages mode)
  if (!supabase) {
  ...
  const serialNumber = `KHT-1447-${String(nextSeq).padStart(3, '0')}`;
...
export async function fetchAdminStats(): Promise<AdminStats> {
  ...
  return {
    ...
    fullQuranMemorizers: list.filter((r) => r.quran_memorization === 'كامل القرآن الكريم (30 جزء)').length
  };
}
